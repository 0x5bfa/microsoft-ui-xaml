...
4. Install the necessary MSBuild / Visual Studio 2022 dependencies. If you already have VS 2022
   ...
   ```
   cd C:\winui3\src
   .\tools\setup\bootstrap\scripts\OneTimeSetup.cmd -Install MSBuild
   ```
...
2. Initialize to build ([details](#initialize-cmd-or-powershell-with-support-for-vs-build-tools)):
   ...
   Or, for PowerShell:
   ```
   .\tools\setup\init\scripts\init.ps1
   ```

3. Build the project:
   ```
   .\tools\build\scripts\Build.cmd
   ```
...
To do a clean build, you can run add a `/c` parameter:
   ```
   .\tools\build\scripts\Build.cmd /c
   ```
...
One option is to simply retry an incremental build by running `tools\build\scripts\Build.cmd` again. This simple
...
set CL=/MP2
```
And then run the build again with `tools\build\scripts\Build.cmd`.
...
```
.\tools\setup\bootstrap\scripts\OneTimeSetup.cmd -Install LogViewer
```

### Other notes

* `tools\setup\bootstrap\scripts\OneTimeSetup.cmd` currently assumes Visual Studio 2022. However, this project can also be built
