...
or by running `tools\build\scripts\Build.cmd /pack` to create the mock Microsoft.WindowsAppSdk nuget package.
...
that you are patching. To build WinUI with experimental features turned off, pass `/muxfinal` when you call `tools\build\scripts\Build.cmd`.
