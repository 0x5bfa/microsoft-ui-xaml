...
will need to do a more extensive build (such as rerunning tools\build\Build.cmd at the root of the repo). See below for such scenarios.
...
running tools\build\Build.cmd to build. This will build everything and re-create the local WinUI component package.

If you want to manually build the required pieces but still update the local WinUI component package you can use `tools\packaging\pack.component.cmd` to recreate it.

### Updating the Local WinUI Component Package

If you create a new local WinUI component package by running tools\build\Build.cmd you might expect to be able to hit F5 in Visual Studio and have
