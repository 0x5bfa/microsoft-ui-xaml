...
role under `tests/infra/Helix/common/pipeline/tools/scripts`: work-item generation in
...
`tests/infra/Helix/payload/tools/scripts/perf` and
`tests/infra/Helix/payload/tools/scripts/setup`. The Helix test-runner payload assets
are grouped under `tests/infra/Helix/payload/tools/scripts/test`. When constructing
`TestPayload`, these files stay under `scripts\runtime`, `scripts\helix\perf`,
`scripts\helix\setup`, and `scripts\helix\test` instead of being flattened to
the payload root. GitHub agent skill metadata now references these paths
...
package maintenance helpers live under `tools/runtime/packages/appx/scripts`.
...
`eng/packaging/intellisense/generated`. The package project consumes that
directory through `$(IntellisenseFolder)`, while the docs-team drop input lives
with the packaging inputs under `eng/packaging/intellisense/drop`. The update
script remains under `tools/packaging/intellisense/scripts` because it is
manual packaging tooling, not package content.
...
Controls-specific build support files live under `eng/controls/build`. The
shared MIDL props and targets live there, with IDL and controls DLL projects
importing them through `$(MUXCProjectRoot)build`. Native C++/WinRT, CRT,
SDK-version, package-version, shared project-configuration settings,
feature-area selection, inner-loop feature overrides, and shared controls
project imports live there too.
...
External binary selection targets now live under `eng/consumebinaries`, keeping
package payload selection rules with the consumed-binary import pipeline.
...
package IntelliSense generation script also lives under
`tools/packaging/intellisense/scripts`, while the docs-team XML drop input and
checked-in generated XML live under `eng/packaging/intellisense`. The repo root
compatibility wrapper was removed; repo-local callers should use
...
Interactions/PullToRefresh/Materials test projects, and the historical
`*_InteractionTests` folders were moved into this test-owned tree. New controls
generated by the control-generation script now place TestUI, API tests, and
interaction tests under the matching `tests/controls` folders.
