...
```
tools\setup\init\scripts\init.ps1
```
...
> Any time you do a `git pull` you also need to re-run tools\setup\init\scripts\init.cmd
...
| :-------- | :----------   | :----------------- |
| x86       | Debug         | tools\setup\init\scripts\init.cmd  x86chk   |
| x86       | Release       | tools\setup\init\scripts\init.cmd  x86fre   |
| x64       | Debug         | tools\setup\init\scripts\init.cmd           |
| x64       | Debug         | tools\setup\init\scripts\init.cmd  x64chk   |
| x64       | Release       | tools\setup\init\scripts\init.cmd  x64fre   |
| arm64     | Debug         | tools\setup\init\scripts\init.cmd  arm64chk |
| arm64     | Release       | tools\setup\init\scripts\init.cmd  arm64fre |
...
`tools\setup\init\scripts\init.cmd` is also responsible for controlling what .NET version the build targets. By default, it is .NET 6.
However, it can be controlled by passing in either `net6` or `net7` as an argument (e.g. `tools\setup\init\scripts\init.cmd net6` will
initialize the build to target .NET 6, `tools\setup\init\scripts\init.cmd x86fre net7` will initialize the build for x86fre targeting
...
To control the .NET version being targeted, `tools\setup\init\scripts\init.cmd` calls the
[SetupDotNetFiles.cmd](../../tools/setup/init/scripts/SetupDotNetFiles.cmd) script to deploy relevant files for targeting either .NET 6 or .NET 7.
...
```
tools\build\scripts\Build.cmd
```

Notes:
+ This does an `msbuild /m` on `src\runtime\Microsoft.UI.Xaml.Runtime.slnx` and `src\controls\solutions\MUXControls.slnx`
...
For more information on the various ways to build, run `tools\build\scripts\Build.cmd /?`. There are predefined "targets" that specify
