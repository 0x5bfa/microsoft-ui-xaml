...
would be starting a full build by issuing `tools\build\scripts\Build.cmd`. If all the changes are correct, the build will proceed just fine.
...
Running the build under the debugger was far more helpful.  `tools/build/scripts/msb.cmd` was modified as follows:
...
`msb <repo>\tests\runtime\tools\customTypes\customTypes.vcxproj`
