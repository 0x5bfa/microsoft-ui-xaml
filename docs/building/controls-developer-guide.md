...
1. From the same command prompt where you ran tools\setup\init\init.cmd and tools\build\Build.cmd, navigate to controls folder and launch muxcontrols.sln file. This will open the controls solution which includes the WinUI controls project and tests in visual studio with the appropriate environment variables setup.

2. Switch the build configuration in visual studio to match how you ran tools\setup\init\init.cmd. By default, tools\setup\init\init.cmd uses x86 chk, so choose x86 and debug configuration in visual studio.
...
### Try these fixes
a. Ensure that you launched muxcontrols.sln file after running tools\setup\init\init.cmd which sets up the appropriate environment variables.
