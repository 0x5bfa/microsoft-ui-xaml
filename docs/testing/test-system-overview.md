...
4. We discover the tests from the build and generate the HelixWorkItems. This is done by [**GenerateWinUIHelixWorkItems.ps1**](../../tests/infra/Helix/common/pipeline/tools/GenerateWinUIHelixWorkItems.ps1).
5. The TestPayload and the work items xml is published to the Pipeline as an artifact.
6. We execute batches of the test work items in parallel on agent VMs that are running the version of Windows that we want to target. We execute [**RunTestPassSliceOnBuildAgent.ps1**](../../tests/infra/Helix/common/pipeline/tools/RunTestPassSliceOnBuildAgent.ps1) on these test machines.
  ...
  * Create the test payload.  **WinUI-CreateTestPayload-Job.yml**
    ...
    * **WinUI-CreateHelixProjFile-Steps.yml**
       * Call **GenerateWinUIHelixWorkItems.ps1**
         * Call **pipeline/tools/GenerateHelixWorkItems.ps1**
  ...
  * **WinUI-RunTestPassOnPipeline-Job.yml**
    * Runs 20 agents in parallel.  On each agent, we:
      ...
      * Call **RunTestPassSliceOnBuildAgent.ps1**
        ...
        * Read proj file that describes test command to run for this agent (created earlier by **pipeline/tools/GenerateHelixWorkItems.ps1**)
...
[**GenerateWinUIHelixWorkItems.ps1**](../../tests/infra/Helix/common/pipeline/tools/GenerateWinUIHelixWorkItems.ps1),
which calls the shared [**GenerateHelixWorkItems.ps1**](../../tests/infra/Helix/common/pipeline/tools/GenerateHelixWorkItems.ps1).
...
Implementation: [**UpdateUnreliableTests-Pipeline.ps1**](../../tests/infra/Helix/common/pipeline/tools/UpdateUnreliableTests-Pipeline.ps1)
