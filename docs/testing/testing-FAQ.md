...
2. Run **tests\infra\payload\tools\scripts\create\CreateTestPayload.ps1** This will produce a TestPayload directory at the repo root.
...
The steps are the same as for local builds, except instead of running `tools\build\scripts\Build.cmd` and `tests\infra\payload\tools\scripts\create\CreateTestPayload.ps1`, you can
...
The tests get run the same way as the functional tests. The only difference is when you call `tests\infra\payload\tools\scripts\create\CreateTestPayload.ps1`,
