...
When calling tools\build\Build.cmd you can pass `samples` to also build the sample apps. You can also invoke msbuild directly
...
The steps are:
1. Build the product and test binaries as normal (tools\build\Build.cmd without args is generally sufficient)
