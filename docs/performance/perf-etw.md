...
Xaml generated these manifests based on codegen (under [tools\runtime\XCPTypesAutoGen\XamlGen\EtwEvents.man](../../tools/runtime/XCPTypesAutoGen/XamlGen/EtwEvents.man)), which gets combined with string resources and turned into a manifest under [src\runtime\xcp\plat\win\desktop\Microsoft-Windows-XAML-ETW.man](../../src/runtime/xcp/plat/win/desktop/Microsoft-Windows-XAML-ETW.man), which then gets built into the WinUI binary. The manifest file can also be used to manually install events using the wevtutil tool.
