...
* Go to `perf\scenarios` directory and execute `tools\setup\init\scripts\init.cmd <build-version>`.  If you'd like to use local build, don't specify `<build-version>` (but be sure to tools\build\scripts\Build.cmd & pack.cmd first).
* Run `msb MeasureMUX-set.slnx` or the solution which contains your measurement app.
