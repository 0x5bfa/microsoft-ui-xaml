...
* Go to `perf\scenarios` directory and execute `tools\setup\init\commands\init.cmd <build-version>`.  If you'd like to use local build, don't specify `<build-version>` (but be sure to tools\build\commands\Build.cmd & pack.cmd first).
