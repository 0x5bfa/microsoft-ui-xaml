...
This is the consumer-side companion to [framework-side changes](framework-side-changes.md). That document covers
...
+ WUC spine are suppressed producer-side (§10 of [framework-side changes](framework-side-changes.md) /
...
- **Runtime match:** to see labels/handles/WUC data, the target must load a
  runtime built with the framework-side changes (see [framework-side changes](framework-side-changes.md)
...
| Doc | Covers |
| --- | --- |
| [framework-side-changes.md](framework-side-changes.md) | **The producer/runtime half — what all these events are** |
