...
It is the WPA counterpart to the standalone [`XamlTelemetryViewerWInui3`](../../../../perf/tooling/XamlTelemtryViewerWInui3/) viewer — same regions, same pairing logic, same labels — but rendered inside WPA so you get its zoom, pivoting, and time-axis tools for free.
...
- **Track 1 — Launch Timeline (`1. Timeline (Track 1)`)**
  ...
  - Detects launch *regions* (markers + phases) declared in the shared [`XamlTimeline/Regions/XamlAppLaunch.regions.xml`](../../../../perf/tooling/XamlTimeline/Regions/XamlAppLaunch.regions.xml): `UserClick`, `Pre-Process`, `ProcessStart`, `Packaged App Init`, `Native Bring-up`, `XAML`, `App ready to interact`, etc.
...
- **WinUI-viewer-equivalent rendering** — events are loaded with `Microsoft.Windows.EventTracing` (TDH), and the event model, region parsing, Track 1 detection, and the event-pairing rules are **shared** with the viewer via the [`XamlTimeline`](../../../../perf/tooling/XamlTimeline/) library, so the same trace produces the same values in both tools.
...
> **[XamlTimeline](../../../../perf/tooling/XamlTimeline/)** `netstandard2.0` library (referenced via
...
> [`XamlTimeline`](../../../../perf/tooling/XamlTimeline/) library. `dotnet publish` (run by `Pack.ps1`)
...
- **.NET SDK** (the project targets `netstandard2.0`; any current SDK works). The build also
  compiles the referenced [`XamlTimeline`](../../../../perf/tooling/XamlTimeline/) project automatically.
...
The `.ptix` is the single-file artifact you hand to **WPA → Manage → Plugins → Install Plugin…**. Build it with [`Pack.ps1`](../../../../perf/tooling/AppBootingVisualizerPlugin/AppBootingVisualizerPlugin/Pack.ps1):
...
The script writes `AppBootingVisualizerPlugin-<version>.ptix` next to itself (≈ 19 MB). `<version>` is read from [`pluginManifest.json`](../../../../perf/tooling/AppBootingVisualizerPlugin/AppBootingVisualizerPlugin/pluginManifest.json).
...
Bumping the version is a one-line edit in [`pluginManifest.json`](../../../../perf/tooling/AppBootingVisualizerPlugin/AppBootingVisualizerPlugin/pluginManifest.json):
...
The region detection is fully data-driven by a **single shared file**. To add / tweak a region, edit [`XamlTimeline/Regions/XamlAppLaunch.regions.xml`](../../../../perf/tooling/XamlTimeline/Regions/XamlAppLaunch.regions.xml) — the same file the WinUI viewer uses, so both tools stay in sync. It is a standard **WPA Regions of Interest** file (so it also opens directly in WPA via *Trace > Trace Properties > Add*); tool-only data WPA doesn't act on is carried in the standard `<Metadata>` node, which WPA ignores for processing:
...
|------|----------|---------|
| **XamlTelemetryViewerWInui3** | [`perf/tooling/XamlTelemtryViewerWInui3`](../../../../perf/tooling/XamlTelemtryViewerWInui3/) | Standalone WinUI 3 viewer — same parser, same regions, same pairing. Use it for ad-hoc browsing without WPA. |
