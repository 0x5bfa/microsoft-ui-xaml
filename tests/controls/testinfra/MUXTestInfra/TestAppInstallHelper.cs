            ...
            if (!TestAppxInstalled.Contains(packageFamilyName))
            {
                ...
                if (packageFile?.Exists == true)
                {
                    ...
                    foreach (var installedPackage in installedPackages)
                    {
                        ...
                        //
                        // Note - Make sure that this list stays in sync with tests\infra\payload\tools\scripts\runtime\runtests.ps1 and
                        //        tests\infra\Helix\payload\tools\scripts\setup\TestPass-OneTimeMachineSetup.ps1
