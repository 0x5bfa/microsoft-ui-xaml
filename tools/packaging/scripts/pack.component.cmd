...
if EXIST "%RepoRoot%\pack.cmd" (
    call "%_root%\tools\common\scripts\PowershellWrapper.cmd" "%_root%\tools\packaging\scripts\build-nupkg.ps1" -Nuspec Microsoft.WindowsAppSDK.WinUI.nuspec %_versionOption% -PackageRoot "%_root%\BuildOutput\packaging\%Configuration%" -InstallPackage
) else (
    call "%_root%\tools\common\scripts\PowershellWrapper.cmd" "%_root%\tools\packaging\scripts\build-nupkg.ps1" -Nuspec Microsoft.WindowsAppSDK.WinUI.nuspec %_versionOption% -PackageRoot "%_root%\BuildOutput\packaging\%Configuration%" -InstallPackage -UseDependencyOverrides
)
if ERRORLEVEL 1 (
    echo ---
