...
.EXAMPLE
    .\tools\setup\init\commands\initrun.ps1 build.cmd mux /q
    .\tools\setup\init\commands\initrun.ps1 msb "src\runtime\xcp\dxaml\dllsrv\winrt\native\Microsoft.ui.xaml.vcxproj" /q
    .\tools\setup\init\commands\initrun.ps1 -Flavor arm64fre build.cmd mux /q
    .\tools\setup\init\commands\initrun.ps1 run-tests-vm.ps1 *CommandBar*
#>
param(
    [Alias("i")]
...
try {
    ...
    Write-Host "Run a full init first: .\tools\setup\init\commands\init.ps1 $Flavor" -ForegroundColor Red
    exit 1
}
...
if ($resolved) {
    $cmd = $resolved.Source
} elseif (Test-Path (Join-Path $repoRoot $cmd)) {
    $cmd = Join-Path $repoRoot $cmd
}
