...
if "%EnvCheck%"=="true" (
    if not exist "%RepoRoot%\packages" (
        ...
        echo.
        echo        Run a full init first:  tools\setup\init\scripts\init.cmd [flavor]
        echo        Example:                tools\setup\init\scripts\init.cmd amd64chk
        exit /b 1
    )
...
goto :SkipDevCmd
:NeedDevCmd
    echo DevEnvDir environment variable not set or msbuild unavailable. Running tools\setup\shell\scripts\DevCmd.cmd to get a developer command prompt...
    if "%ARM64EC%"=="1" (
        call %RepoRoot%\tools\setup\shell\scripts\DevCmd.cmd /PreserveContext /prerelease -arch=amd64 -host_arch=amd64
    ) else if "%ARM64%"=="1" (
        call %RepoRoot%\tools\setup\shell\scripts\DevCmd.cmd /PreserveContext /prerelease -arch=arm64 -host_arch=amd64
    ) else (
        call %RepoRoot%\tools\setup\shell\scripts\DevCmd.cmd /PreserveContext /prerelease -arch=%_BuildArch% -host_arch=amd64
    )
...
set PATH=%RepoRoot%\.buildtools\MSBuild\Current\Bin\amd64;%RepoRoot%\.tools;%RepoRoot%\.tools\VSS.NuGet.AuthHelper;%RepoRoot%\tools;%RepoRoot%\tools\build\scripts;%RepoRoot%\tools\clang\scripts;%PATH%
...
call %RepoRoot%\tools\setup\init\scripts\SetupDotNetFiles.cmd %RepoRoot%

powershell -ExecutionPolicy Bypass -NoProfile -File "%RepoRoot%\tools\setup\init\scripts\GenerateTestPfx.ps1" "%BuildArtifactsDir%\certificates\WinUITest.pfx"
...
if "%EnvOnly%"=="" (
    ...
    powershell -ExecutionPolicy Bypass -NoProfile -File %RepoRoot%\tools\setup\init\scripts\Initialize-Restore.ps1 -RepoRoot %RepoRoot% %Verbose%
)
...
doskey /macrofile=%RepoRoot%\tools\setup\init\scripts\aliases
