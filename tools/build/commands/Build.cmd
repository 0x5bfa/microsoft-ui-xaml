...
pushd "%_repoRoot%"
set _initCmd=%_repoRoot%\tools\setup\init\commands\init.cmd
set _packComponentCmd=%_repoRoot%\tools\packaging\commands\pack.component.cmd
...
if "%1"=="/c" (
    ...
    rem Inline init: run tools\setup\init\commands\init.cmd <flavor> /envcheck before building.
...
if not "%_initFlavor%" == "" (
    ...
    if ERRORLEVEL 1 (
        echo ERROR: tools\setup\init\commands\init.cmd %_initFlavor% /envcheck failed
        exit /b 1
    )
) else if "%EnvironmentInitialized%" == "" (
    echo Please run tools\setup\init\commands\init.cmd or use /i ^<flavor^> to initialize the build environment
    exit /b 1
)
...
if "%_clean%"=="1" (
    call :callScript "%RepoRoot%\tools\build\commands\clean.cmd" /all
    set _restore=1
)
...
echo                        Runs tools\setup\init\commands\init.cmd with /envcheck so no restore is performed. Use when
