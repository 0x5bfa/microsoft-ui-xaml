...
pushd "%_repoRoot%"
set _initCmd=%_repoRoot%\tools\setup\init\scripts\init.cmd
set _packComponentCmd=%_repoRoot%\tools\packaging\scripts\pack.component.cmd
...
if "%1"=="/c" (
    ...
    rem Inline init: run tools\setup\init\scripts\init.cmd <flavor> /envcheck before building.
...
if not "%_initFlavor%" == "" (
    ...
    if ERRORLEVEL 1 (
        echo ERROR: tools\setup\init\scripts\init.cmd %_initFlavor% /envcheck failed
        exit /b 1
    )
) else if "%EnvironmentInitialized%" == "" (
    echo Please run tools\setup\init\scripts\init.cmd or use /i ^<flavor^> to initialize the build environment
    exit /b 1
)
...
if "%_clean%"=="1" (
    call :callScript "%RepoRoot%\tools\build\scripts\clean.cmd" /all
    set _restore=1
)
...
if EXIST "%reporoot%\src\compiler\BuildTasks\Microsoft\Lmr\XamlTypeUniverse.cs" (
    ...
    call :buildSolution %reporoot%\src\compiler\XamlCompilerPrerequisites.slnx
    ...
    call :buildSolution %reporoot%\eng\xamlcompiler\XamlCompilerPublic.csproj
    if ERRORLEVEL 1 goto:showDurationAndExit
...
if "%_targetMux%" == "1" (
   ...
   call :buildSolution %reporoot%\src\runtime\Microsoft.UI.Xaml.Runtime.slnx
   if ERRORLEVEL 1 goto:showDurationAndExit
   if not "%_nomock%"=="1" call :buildMockPackage
   call :buildSolution %reporoot%\src\controls\MUXControls.slnx /restore
...
echo                        Runs tools\setup\init\scripts\init.cmd with /envcheck so no restore is performed. Use when
