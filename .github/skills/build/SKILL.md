...
```powershell
# Always wrap commands with .\tools\setup\init\scripts\initrun.ps1 — it sets up the build environment automatically.
# Default flavor is amd64chk. Override with -Flavor.

.\tools\setup\init\scripts\initrun.ps1 .\tools\build\scripts\Build.cmd /q                  # full repo build (product + tests) — USE THIS BY DEFAULT
.\tools\setup\init\scripts\initrun.ps1 .\tools\build\scripts\Build.cmd /q product          # product code only (no tests)
.\tools\setup\init\scripts\initrun.ps1 .\tools\build\scripts\Build.cmd /q mux              # MUX only (Microsoft.UI.Xaml.dll)
.\tools\setup\init\scripts\initrun.ps1 msb /q "path\to\project.vcxproj"            # build a single project
.\tools\setup\init\scripts\initrun.ps1 -Flavor arm64fre .\tools\build\scripts\Build.cmd /q # build for a different flavor
```
...
**Rules:**
- Always prefix with `.\tools\setup\init\scripts\initrun.ps1`
- Always pass `/q` for quiet output (errors only)
- Set `initial_wait` to at least **300 seconds** — builds take 1-10+ minutes
- **When the user asks to "build the repo" or just "build" without specifying a target, use `.\tools\setup\init\scripts\initrun.ps1 .\tools\build\scripts\Build.cmd /q` (full build).**
...
|---------|---------------|------|
| `.\tools\setup\init\scripts\initrun.ps1 .\tools\build\scripts\Build.cmd /q` | Everything (product + tests) | 10+ min |
| `.\tools\setup\init\scripts\initrun.ps1 .\tools\build\scripts\Build.cmd /q mux` | `Microsoft.UI.Xaml.dll` only | 1-6 min |
| `.\tools\setup\init\scripts\initrun.ps1 .\tools\build\scripts\Build.cmd /q product` | Product code (no tests) | 5-10 min |
| `.\tools\setup\init\scripts\initrun.ps1 .\tools\build\scripts\Build.cmd /q /c` | Clean + full rebuild | 15+ min |
| `.\tools\setup\init\scripts\initrun.ps1 msb /q "<project>"` | Single `.vcxproj` | 5s - 5 min |

### Flags (for `.\tools\build\scripts\Build.cmd`)
...
|---|---|
| `src/runtime/xcp/**` (source only) | **bt:** `bt build` · MSBuild: `.\tools\setup\init\scripts\initrun.ps1 msb /q "src\runtime\xcp\dxaml\dllsrv\winrt\native\Microsoft.ui.xaml.vcxproj"` |
| `src/controls/**` (source only) | **bt:** `bt build` · MSBuild: `.\tools\setup\init\scripts\initrun.ps1 msb /q "src\controls\dll\Microsoft.UI.Xaml.Controls.vcxproj"` |
| `tests/runtime/native/external/<area>/**` (source only) | **bt:** `bt build` · MSBuild: `.\tools\setup\init\scripts\initrun.ps1 msb /q "tests\runtime\native\external\<area>\Microsoft.UI.Xaml.Tests.External.<Area>.vcxproj"` |
| `.vcxproj`, `.vcxitems`, `.props`, `.targets`, NuGet deps | `.\tools\setup\init\scripts\initrun.ps1 .\tools\build\scripts\Build.cmd /q` (MSBuild only — do NOT use bt) |
| Multiple areas or unsure | `.\tools\setup\init\scripts\initrun.ps1 .\tools\build\scripts\Build.cmd /q` |
...
1. Use the `/b` flag in `build.cmd` which sets `/m:2` (2 parallel processes):
   ```powershell
   .\tools\setup\init\scripts\initrun.ps1 .\tools\build\scripts\Build.cmd /q /b
   ```
...
3. If it keeps failing, stale PCH files from a previous build with a different compiler version may be the cause. Do a clean build:
   ```powershell
   .\tools\setup\init\scripts\initrun.ps1 .\tools\build\scripts\Build.cmd /q /c /b
   ```
