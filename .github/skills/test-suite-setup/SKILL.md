...
- A **full repo build** (product + tests): `.\tools\setup\init\scripts\initrun.ps1 .\tools\build\scripts\Build.cmd /q /b`
- The `/b` flag is recommended for test-suite prep to avoid PCH memory exhaustion on limited-memory machines.
- For a clean first-time build, use `/c` as well: `.\tools\setup\init\scripts\initrun.ps1 .\tools\build\scripts\Build.cmd /q /c /b`
...
```powershell
.\tools\setup\init\scripts\initrun.ps1 .\tools\build\scripts\Build.cmd /q /b   # builds product + tests
```
